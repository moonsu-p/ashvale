...
import { isDowned, canEscort } from '@/systems/explore';
...
export function ExplorePanel({ state }: { state: GameState }) {
  ...
  const [escort, setEscort] = useState<string | null>(null);
  ...
  const escorts = Object.values(state.companions).filter((c) => canEscort(state, c.id));
    ...
    <section>
      ...
      {escorts.length > 0 && (
        <div className="mb-1 flex flex-wrap items-center gap-1 text-xs">
          <span style={{ color: PALETTE.inkSoft }}>동행</span>
          <button
            onClick={() => setEscort(null)}
            className="rounded px-2 py-0.5"
            style={{ background: escort === null ? PALETTE.gold : PALETTE.paperDim, color: PALETTE.ink }}
          >
            없음
          </button>
          {escorts.map((c) => (
            <button
              ...
              className="rounded px-2 py-0.5"
              style={{ background: escort === c.id ? PALETTE.gold : PALETTE.paperDim, color: PALETTE.ink }}
            >
              {c.name}
            </button>
          ))}
